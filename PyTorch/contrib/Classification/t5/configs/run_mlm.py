...
class DataTrainingArguments:
    ...
    disable_progress_bar: bool = field(
        default=False, metadata={"help": "Whether to disable the progress bar."}
    )#禁止进度条显示

def main():
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments))
    model_args, data_args, training_args = parser.parse_args_into_dataclasses()
    training_args.disable_progress_bar = True  # 设置为 True 来禁用进度条
    ...
    data_collator = DataCollatorForLanguageModeling(
        tokenizer=tokenizer, mlm=False, mlm_probability=data_args.mlm_probability
    )
    ...
    trainer = Trainer(
        ...
        callbacks=[IterationLogCallback(os.path.join(training_args.output_dir, "cuda.log"))], 
    )
